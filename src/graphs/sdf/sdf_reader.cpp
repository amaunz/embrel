...
void SDFReader::configure()
{
	...
	setInputFiles(files);
	if(mGraphs.size()==0){
		if(fs::exists(gCfg().getOutputFile(basen + "-graphs.ser")) && gCfg().getBool("SDFReader.use_cache")){
			ifstream ifs(gCfg().getOutputFile(basen + "-graphs.ser").c_str());
...
void SDFReader::setInputFiles(const std::string& inputfileline)
{
	...
	BOOST_FOREACH(const string& pair, file_class_pairs){
		...
		if(!fs::exists(fd.name)){
			cerr << "Warning: SDFReader: File `"<<fd.name<<"' does not exist."<<endl;
			exit(1);
		}
...
void SDFReader::readInputFiles()
{
	...
	BOOST_FOREACH(const FileDescriptor& fd, mInputFiles){
		if(gCfg().getBool("verbose"))
			cout << "Reading SDF input file `"<<fd.name<<"'..."<<flush;
		ifstream is(fd.name.c_str());
		int fcnt = 0;
		while(readMolekule(is, fd.classid, graphCount++) && ++fcnt<max_num);
		if(gCfg().getBool("verbose"))
			cout << graphCount << " molecules read."<<endl;
	}
